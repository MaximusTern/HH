...
FOUND_RESTRICTION_HH = 200  # ограничение hh.ru на количество выводимых вакансий
...
def currency():
    currec = input('Выберете валюту. EUR, USD, RUR. По-умолчанию будет RUR. : ')
    if currec == 'RUR' or currec == 'USD' or currec == 'EUR':
        return currec
    else:
        print('Ищем RUR')
        return 'RUR'
...
def salary():
    ...
    while True:
        salary_ = input('Введите ЗП для поиска')
        type(salary_) == int
        return salary_
...
if __name__ == '__main__':
    ...
    par = {'text': vacancy_answer, 'area': area_param[0], 'currency': currency(), 'salary': salary()}  # добавляем параметры
    ...
    if r.status_code == 200:
        ...
        if result['items']:
            ...
            info = {'keywords': vacancy_answer, 'count': found, 'requirements': requirements}
